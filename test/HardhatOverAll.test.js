...
contract('Overall Test',  function ([owner, platformFeeRecipient, artist, buyer, bidder1, bidder2, bidder3])  {
    ...
    beforeEach(async function () {
        ...
        this.gtonShopNFT = await GTONShopNFT.new(platformFeeRecipient, platformFee);
        ...
        await this.gtonShopAddressRegistry.updateGTONShopNFT(this.gtonShopNFT.address);
    ...
    describe('Minting and auctioning NFT', function() {
        ...
        it('Scenario 1', async function(){
            ...
            console.log(`
            Now minting...`);
            let result = await this.gtonShopNFT.mint(artist, 'http://artist.com/art.jpeg', {from: artist, value: ether(PLATFORM_FEE)});
            console.log(`
            Minted successfully`);
            ...
            *Event Minted should be emitted with correct values: 
            ...
            let listing = await this.gtonShopMarketplace.listings(this.gtonShopNFT.address, new BN('1'), artist);
            console.log(`
        ...
        it('Scenario 2', async function() {
            ...
            *Event Minted should be emitted with correct values: 
            ...
            console.log(`
            The artist approves the nft to the market`);
            await this.gtonShopNFT.setApprovalForAll(this.gtonShopAuction.address, true, {from: artist});
            ...
            *Event AuctionCreated should be emitted with correct values: 
            nftAddress = ${this.gtonShopNFT.address}, 
            tokenId = 1, 
            payToken = ${this.mockERC20.address}`);
        ...
        it('Scenario 3', async function() {
            ...
            *Event Minted should be emitted with correct values: 
            ...
            result = await this.gtonShopNFT.mint(artist, 'http://artist.com/art2.jpeg', {from: artist, value: ether(PLATFORM_FEE)});
            console.log(`
            ...
            *The difference of the recipients's GCD balance should be ${PLATFORM_FEE*2} GCDs as the platform fee is ${PLATFORM_FEE} GCDs `);
            ...
            await this.gtonShopBundleMarketplace.listItem(
                    'mynfts',
                    [this.gtonShopNFT.address, this.gtonShopNFT.address],
                    [new BN('1'),new BN('2')],
                    [new BN('1'), new BN('1')],
                    this.mockERC20.address,
                    ether('20'),
            ...
            *The nfts should be on the bundle marketplace listing`);
            ...
            expect(listing.nfts[0]).to.be.equal(this.gtonShopNFT.address);
            expect(listing.nfts[1]).to.be.equal(this.gtonShopNFT.address);
            ...
            *The two nfts now should belong to buyer`);
            let nftOwner = await this.gtonShopNFT.ownerOf(new BN('1'));
            expect(nftOwner).to.be.equal(buyer);
            nftOwner = await this.gtonShopNFT.ownerOf(new BN('2'));
            expect(nftOwner).to.be.equal(buyer);
